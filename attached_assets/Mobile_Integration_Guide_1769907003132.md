...
For mobile apps, we use Google's native ID token flow instead of browser-based OAuth. This provides a better UX with native Google Sign-In sheets.

```typescript
import * as Google from 'expo-auth-session/providers/google';
import { Platform } from 'react-native';

// Configure with your Google OAuth client IDs
...
async function signInWithGoogle() {
  ...
  if (result.type === 'success') {
    // Exchange Google ID token for Supabase session via edge function
    ...
    // Set the Supabase session with returned tokens
...
**Important Notes:**
- The `mobile-google-auth` edge function handles token validation and user creation
- New users automatically receive 5 free credits and are synced to marketing automation
- The ID token approach avoids browser redirects, providing a native feel
- Tokens are short-lived; the Supabase client handles automatic refresh
