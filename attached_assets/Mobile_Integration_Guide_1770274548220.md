...
```
┌─────────────┐     ┌──────────────────────┐     ┌──────────────────────┐     ┌─────────────┐
│  Mobile App │────►│  profiles table      │     │  Edge Functions      │────►│ Expo Push   │
│  (Expo)     │     │  expo_push_token     │◄────│  (on brief complete) │     │ Service     │
└─────────────┘     └──────────────────────┘     └──────────────────────┘     └─────────────┘
```
...
| `completed` | ✅ Pipeline finished successfully | Brief ready to view/listen |
| `failed` | ❌ Pipeline failed (no transcript) | Show "Retry Brief" button |
| `summary_failed` | ⚠️ Summary quality check failed | Show "Regenerate Summary" button |
...
// Response statuses:
// - status: 'completed' + transcript: string → Transcript ready, proceed to summarization
// - status: 'processing' → Still processing, check back later
// - status: 'not_available' → Transcript not available for this episode
```
...
5. If retry still fails quality gate:
   → Set pipeline_status = 'summary_failed'
...
async function handleRegenerateSummary(masterBriefId: string) {
  ...
  // Pipeline goes: summarizing → recording → completed (or summary_failed)
}
```
...
|---------|----------|-------|
| Display different UI based on `pipeline_status` | 🔴 Critical | `failed`, `summary_failed`, `completed` |
| "Retry Brief" button for `failed` status | 🔴 Critical | Calls `retry-taddy-transcript` |
| "Regenerate Summary" button for `summary_failed` | 🔴 Critical | Calls `regenerate-summary` |
| Store `expo_push_token` in profiles | 🔴 Critical | For push notifications |
| Handle notification deep links | 🔴 Critical | Navigate to brief |
| Poll or Realtime for status updates | 🟡 Medium | 3-5 second polling during generation |
| Auto-retry stale transcripts (>2min) | 🟡 Medium | Web does this automatically |
