...
For native mobile apps, use the `mobile-google-auth` edge function to exchange a Google ID token for a Supabase session.

#### `mobile-google-auth` - Exchange Google ID Token

| Property | Value |
|----------|-------|
| **Endpoint** | `POST /functions/v1/mobile-google-auth` |
| **Auth Required** | No (ID token provides authentication) |
...
**Behavior:**
- New users: Profile created with 5 free credits, synced to Loops
- Existing users: Returns session immediately
...
|----------|------|----------|---------|
| `mobile-google-auth` | No* | Auth | Mobile Google Sign-In |
